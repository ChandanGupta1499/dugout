...
import { getSessionStatus, pauseSim, resumeSim, setSimSpeed, startSim, stopSim, } from './scheduler.js';
...
simRouter.post('/sim/start', (req, res) => {
    try {
        ...
        const status = startSim({
            ...
            kickoffOffsetMs: kickoffOffsetMs != null ? Number(kickoffOffsetMs) : undefined,
        });
        res.json(status);
    }
    catch (error) {
