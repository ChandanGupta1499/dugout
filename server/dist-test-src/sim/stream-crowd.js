...
export function crowdUserId(author) {
    return `yt-${shortHash(author)}`;
}
const upserted = new Set();
const ensuredChannels = new Set();
async function ensureChannel(matchId, userId) {
...
export async function sendCrowdMessage(matchId, author, text) {
